...
def evaluate_formula(formula, var_dict):
    ...
    """
    # 预处理：将 log(x,e) 转换为 ln(x) 或 math.log(x)
    # 先处理 log(表达式, e) 的情况
    def replace_log_with_e(match):
        inner_expr = match.group(1)  # 获取第一个参数
        # 将 log(x,e) 转换为 ln(x)
        return f'ln({inner_expr})'
    
    # 使用正则表达式匹配 log(..., e) 形式
    # 注意：这里假设 e 是自然对数的底数，而不是变量 e
    pattern_log_e = r'log\s*\(\s*([^,]+)\s*,\s*e\s*\)'
    formula = re.sub(pattern_log_e, replace_log_with_e, formula)
    
    # 然后替换数学函数，确保函数名不会被变量替换影响
    for math_func, py_func in math_func_map.items():
        # 使用正则表达式只替换独立的函数名（后面跟着括号）
        pattern = r'\b' + re.escape(math_func) + r'\s*\('
        formula = re.sub(pattern, f'math.{py_func}(', formula)
    
    # 然后替换变量名，使用正则表达式确保只替换独立的变量名
    # 特别注意：需要先检查是否有名为 'e' 的变量，如有，需要特殊处理
    e_is_var = 'e' in var_dict
    
    for var_name, var_value in var_dict.items():
        # 处理变量 e 的特殊情况
        if var_name == 'e' and e_is_var:
            # 对于变量 e，需要确保不替换数学常数 e
            # 使用更复杂的正则表达式，确保 e 不是数学常数或函数参数
            # 匹配独立的 e（前后不是字母数字或下划线，且不在函数调用中）
            pattern = r'(?<![a-zA-Z0-9_.])\b' + re.escape(var_name) + r'\b(?![a-zA-Z0-9_.])'
    ...
    if not e_is_var:
        # 如果 e 不是变量，添加数学常数 e
        formula = re.sub(r'(?<![a-zA-Z0-9_.])\be\b(?![a-zA-Z0-9_.])', 'math.e', formula)
...
def process_question(question):
    ...
    if 'e' in var_dict:
        print(f"  注意：存在名为 'e' 的变量，值为: {var_dict['e']}")
    ...
    for idx, answer in enumerate(correct_answers, 1):
        ...
        if formula:
            ...
            if raw_value is not None:
                ...
                # 先处理 log(x,e)
                pattern_log_e = r'log\s*\(\s*([^,]+)\s*,\s*e\s*\)'
                debug_formula = re.sub(pattern_log_e, r'ln(\1)', debug_formula)
                
                # 再替换变量
                for var_name, var_value in var_dict.items():
                    if var_name == 'e' and 'e' in var_dict:
                        # 对于变量 e，需要特殊处理
                        pattern = r'(?<![a-zA-Z0-9_.])\b' + re.escape(var_name) + r'\b(?![a-zA-Z0-9_.])'
                ...
                for math_func, py_func in math_func_map.items():
                    pattern = r'\b' + re.escape(math_func) + r'\s*\('
                    debug_formula = re.sub(pattern, f'{math_func}(', debug_formula)
                
                print(f"  空白 {display_index} :")
                print(f"    原始公式: {formula}")
                print(f"    处理后公式: {debug_formula}")
